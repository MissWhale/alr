var db=require('../../db');
exports.boardlist=(req,res)=>{ //메인페이지
        var max=10; //페이지당 문제수
        var pno=req.query.page; //페이지넘버
        if(!pno)  pno=1;
        var start=max*pno-max;
        var no=req.session.mem_no
        sql='select count(*) as count from board where brd_notice=0'
        db.query(sql,(err,result)=>{
                if(err) console.log(err)
                else{
                        var count=result[0].count
                        sql="select *,(select count(*) from board_comment b where b.brd_no=board.brd_no) as com_cnt,(select count(*) from board_like b where b.brd_no=board.brd_no) as brd_like from board where brd_notice=0 order by brd_no desc limit ?,?";
                                        ...
                                        if(no){
                                                ...
                                                db.query(sql,no,(err,user)=>{
                                                        if(err) console.log(err)
                                                                ...
                                                                if(no==1){
                                                                        res.render('boardlist',{log:1,user:user[0],list:board,pager:pager,pno:pno})
                                                                }else{
                                                                        res.render('boardlist',{log:2,user:user[0],list:board,pager:pager,pno:pno})
                                                                }
                                                        }
                                                })   
                                        }else{
                                                res.render('boardlist',{log:0,list:board,pager:pager,pno:pno})
                                        }
                                }
                        })
                }
        ...
        if(!no){
                ...
                db.query(sql,no,(err,user)=>{
                        if(err) console.log(err)
                                ...
                                if(no==1){
                                        res.render('boardwrite',{log:1,user:user[0]})
                                }else{
                                        res.render('boardwrite',{log:2,user:user[0]})
                                }
                        }
                }) 
        }
...
exports.write=(req,res)=>{
        ...
        var no=req.session.mem_no
        var sql='insert into board(mem_no,brd_title,brd_text) values(?,?,?)'
                        ...
                        if(err) console.log(err)
                                ...
                                db.query(sql,no,(err,user)=>{
                                        if(err) console.log(err)
                                                ...
                                                if(no==1){
                                                        res.render('boardupdate',{log:1,user:user[0],board:result[0]})
                                                }else{
                                                        res.render('boardupdate',{log:2,user:user[0],board:result[0]})
                                                }
                                        }
                                }) 
                        }
